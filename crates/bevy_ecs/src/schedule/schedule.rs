...
impl ScheduleGraph {
    ...
    fn auto_insert_apply_deferred(
        ...
        for node in &topo {
            ...
            for target in dependency_flattened.neighbors_directed(*node, Direction::Out) {
    ...
    fn map_sets_to_systems(
        ...
        for &id in hierarchy_topsort.iter().rev() {
            ...
            for child in hierarchy_graph.neighbors_directed(id, Direction::Out) {
                match child {
                    NodeId::System(_) => {
                        systems.push(child);
    ...
    fn get_dependency_flattened(&mut self, set_systems: &HashMap<NodeId, Vec<NodeId>>) -> DiGraph {
        ...
        for (&set, systems) in set_systems {
            if systems.is_empty() {
                ...
                for a in dependency_flattened.neighbors_directed(set, Direction::In) {
                    for b in dependency_flattened.neighbors_directed(set, Direction::Out) {
    ...
    fn build_schedule_inner(
        ...
        for &sys_id in &dg_system_ids {
            let num_dependencies = dependency_flattened_dag
                .graph
                .neighbors_directed(sys_id, Direction::In)
                .count();

            let dependents = dependency_flattened_dag
                .graph
                .neighbors_directed(sys_id, Direction::Out)
...
impl ScheduleGraph {
    ...
    fn anonymous_set_name(&self, id: &NodeId) -> String {
        format!(
            ...
            self.hierarchy
                .graph
                .edges_directed(*id, Direction::Out)
    ...
    fn check_system_type_set_ambiguity(
        ...
        for (&id, systems) in set_systems {
            ...
            if set.is_system_type() {
                ...
                let before = self.dependency.graph.edges_directed(id, Direction::In);
                let after = self.dependency.graph.edges_directed(id, Direction::Out);
    ...
    fn traverse_sets_containing_node(&self, id: NodeId, f: &mut impl FnMut(NodeId) -> bool) {
        for (set_id, _) in self.hierarchy.graph.edges_directed(id, Direction::In) {
            if f(set_id) {
