...
pub use graph::{DiGraph, Direction, Graph, UnGraph};
pub use node::NodeId;
...
impl Default for CheckGraphResults {
    fn default() -> Self {
        Self {
            ...
            transitive_reduction: Graph::new(),
            transitive_closure: Graph::new(),
        }
    }
}
...
pub(crate) fn check_graph(graph: &DiGraph, topological_order: &[NodeId]) -> CheckGraphResults {
    ...
    for (i, &node) in topological_order.iter().enumerate() {
        ...
        for pred in graph.neighbors_directed(node, Direction::In) {
    ...
    for a in topsorted.nodes().rev() {
        ...
        for b in topsorted.neighbors_directed(a, Direction::Out) {
            ...
            if !visited[index_b] {
                ...
                let successors = transitive_closure
                    .neighbors_directed(b, Direction::Out)
                    .collect::<Vec<_>>();
                for c in successors {
