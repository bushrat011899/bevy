...
impl TarjanScc {
    ...
    pub(crate) fn run(&mut self, g: &Graph<true>, mut f: impl FnMut(&[NodeId])) {
        self.nodes.clear();
        self.nodes
            .resize(g.node_count(), NodeData { rootindex: None });

        for n in g.nodes() {
            let visited = self.nodes[g.to_index(n).unwrap()].rootindex.is_some();
            if !visited {
    ...
    fn visit(&mut self, v: NodeId, g: &Graph<true>, mut f: &mut impl FnMut(&[NodeId])) {
        macro_rules! node {
            ($node:expr) => {
                self.nodes[g.to_index($node).unwrap()]
            };
        }
        ...
        if v_is_local_root {
            ...
            let start = self
                ...
                .iter()
                .rposition(|&w| {
                    if nodes[g.to_index(v).unwrap()].rootindex
                        > nodes[g.to_index(w).unwrap()].rootindex
                    {
                        true
                    } else {
                        nodes[g.to_index(w).unwrap()].rootindex = c;
                        indexadjustment += 1;
                        false
                    }
                })
                .map(|x| x + 1)
                .unwrap_or_default();
            nodes[g.to_index(v).unwrap()].rootindex = c;
